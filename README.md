...
-----------------------------------------------------
    uv run python scripts/eval_ablations.py --demo       # Tables 4 + 5
        # or --reuse-traces-from manuscript/traces (after real-LLM run)
...
--------------------------------------------
  manuscript/
    ...
    traces/            Pipeline traces (md + json + baseline +
                       side-by-side comparison) per synthetic patient.
                       Real-LLM (DeepSeek-R1-Distill-Qwen-14B) outputs
                       are committed here. Run `scripts/run_demo.py --demo`
                       to overwrite with mock-mode traces for offline
                       development.
