...
# against the real traces in manuscript/traces/.
#
# Steps:
...
echo "[postprocess] (1/4) eval_ablations.py (real LLM, reusing traces/) ..."
uv run python scripts/eval_ablations.py \
    --reuse-traces-from manuscript/traces

echo "[postprocess] (2/4) sensitivity_sweep.py ..."
uv run python scripts/sensitivity_sweep.py

echo "[postprocess] (3/4) provenance_audit.py (traces/) ..."
uv run python scripts/provenance_audit.py --traces-dir manuscript/traces

echo "[postprocess] (4/4) render_figures.py (traces/) ..."
uv run python scripts/render_figures.py --traces-dir manuscript/traces
